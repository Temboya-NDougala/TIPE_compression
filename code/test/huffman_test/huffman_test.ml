let test_huffman preffix =
  Huffman.zip (preffix^"/"^preffix^".txt");
  print_string (preffix^".txt zipped\n");
  Huffman.unzip (preffix^"/"^preffix^".txt.hff");
  let archive_channel = open_in (preffix^"/"^preffix^"_archive.txt") in
  let unzipped_channel = open_in (preffix^"/"^preffix^".txt") in
